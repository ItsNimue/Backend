...
const PairStatusSchema = new mongoose.Schema({
    ...
    // Only an UNCLAIMED pairing attempt should auto-expire (10 min).
    // Once a session connects, this is cleared so the record — and the
    // bot's linked account — is never silently wiped by Mongo's TTL job.
    expiresAt: { type: Date, default: () => new Date(Date.now() + 600 * 1000) }
});
PairStatusSchema.index({ expiresAt: 1 }, { expireAfterSeconds: 0 });
